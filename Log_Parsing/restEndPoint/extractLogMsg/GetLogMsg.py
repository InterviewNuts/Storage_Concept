...
class GetAllLogs(GetLogMsgBase):
    def __init__(self, argList, TroubleshootingAssistantDir, segregated_log) -> None:
        ...
        if platform.system() == "Linux":
            ...
            self.without_dupTraceFN = self.seg_log_prg_name + \
                "/" + self.caseId + "_TRACE_WITHOUT_DUP_LOG.log"

            self.without_traceFinalFN = self.seg_log_prg_name+"/" + \
            ...
            self.errorFileXMLName = self.seg_log_prg_name + "/" + \
                self.caseId + "_REQUEST_RESPONSE_ERROR_LOG.log"

            self.without_dupWarnFp = self.seg_log_prg_name + \
                "/" + self.caseId + "_WARN_WITHOUT_DUP_LOG.log"

            self.without_dupWarnFinalFp = self.seg_log_prg_name+"/" + \
                "FINAL_" + self.caseId + "_WARN_WITHOUT_DUP_LOG.log"

            self.without_infoFN = self.seg_log_prg_name + \
                "/" + self.caseId + "_INFO_WITHOUT_DUP_LOG.log"

            self.without_infoFinalFN = self.seg_log_prg_name+"/" + \
                "FINAL_" + self.caseId + "_INFO_WITHOUT_DUP_LOG.log"

            self.without_debugFN = self.seg_log_prg_name + \
                "/" + self.caseId + "_DEBUG_WITHOUT_DUP_LOG.log"

            self.without_debugFinalFN = self.seg_log_prg_name+"/" + \
                "FINAL_" + self.caseId + "_DEBUG_WITHOUT_DUP_LOG.log"

            self.without_dupErrFN = self.seg_log_prg_name + \
                "/" + self.caseId + "_ERROR_WITHOUT_DUP_LOG.log"

            self.without_dupErrFinalFN = self.seg_log_prg_name+"/" + \
    ...
    def getUserGivenDurationErrorLog(self):
        ...
        with open(self.errorFileName) as withoutDupErrLogFp:
            for line in self.read_in_chunks(withoutDupErrLogFp):
                self.write_to_user_given_error_log(
                    line, startDate, endDate, matches)
    ...
    def getUserGivenDurationWarnLog(self):
        ...
        print("File size ", os.path.getsize(
            self.seg_log_prg_name + "//" + self.caseId + "_WARN_LOG.log"))

        with open(self.seg_log_prg_name + "//" + self.caseId + "_WARN_LOG.log") as withoutDupErrLogFp:
            for line in self.read_in_chunks(withoutDupErrLogFp):
                self.write_to_user_given_warn_log(
                    line, startDate, endDate, matches)
    ...
    def getUserGivenDurationInfoLog(self):
        ...
        print("File size ", os.path.getsize(
            self.seg_log_prg_name + "//" + self.caseId + "_INFO_LOG.log"))

        with open(self.seg_log_prg_name + "//" + self.caseId + "_INFO_LOG.log") as withoutDupErrLogFp:
            for line in self.read_in_chunks(withoutDupErrLogFp):
                self.write_to_user_given_info_log(
                    line, startDate, endDate, matches)
        ...
        try:
            ...
            if dateFinderFailed:
                ...
                if validdate1:
                    if ((pd.Timestamp(dt).tz_localize(None) >= startDate) and (pd.Timestamp(dt).tz_localize(None) <= endDate)):
                        # print(dt)
                        self.userGivenDurationInfoLogFileFP.write(
                            "%s" % (line))
            # this else for non NLP Date matching ( that is for date finder)
            elif len(matches) > 0:
                dateFound = matches[0]
    ...
    def getUserGivenDurationTraceLog(self):
        ...
        if platform.system() == "Linux":
            userGivenDurationTraceLogFileFP = open(
                self.seg_log_prg_name+"/" + self.caseId + durationLogTraceFileName, "w")
        else:
            userGivenDurationTraceLogFileFP = open(
                self.seg_log_prg_name + "\\" + self.caseId + durationLogTraceFileName, "w")
    ...
    def getUserGivenDurationDebugLog(self):
        ...
        print("File size ", os.path.getsize(
            self.seg_log_prg_name + "//" + self.caseId + "_DEBUG_LOG.log"))

        with open(self.seg_log_prg_name + "//" + self.caseId + "_DEBUG_LOG.log") as withoutDupErrLogFp:
            for line in self.read_in_chunks(withoutDupErrLogFp):
                self.write_to_user_given_debug_log(
                    line, startDate, endDate, matches)
    ...
    def getUserGivenDurationForAllLog(self, *args: str) -> None:
        try:
            ...
            self.errorWarnFile = open(
                self.seg_log_prg_name+"/" + self.caseId + "_allErrorsUserGivenTime.log", "a")
    ...
    def getUserGivenDurationLog(self):
        if self._error:
            self.durationLogErrFileName = 'USERGIVEN_' + \
                self.logTimeDuration + '_MINUTES_ERROR_LOG.log'
            self.userGivenDurationLogFileFP = open(
                self.seg_log_prg_name+"/" + self.caseId + "_" + self.durationLogErrFileName, "w")

            self.getUserGivenDurationErrorLog()
            self.getUserGivenDurationForAllLog(self.userGivenDurationLogFileFP)

        if self._warn:
            self.durationLogWarnFileName = 'USERGIVEN_' + \
                self.logTimeDuration + '_MINUTES_WARN_LOG.log'
            self.userGivenDurationWarnLogFileFP = open(
                self.seg_log_prg_name+"/" + self.caseId + "_" + self.durationLogWarnFileName, "w")

            self.getUserGivenDurationWarnLog()
            self.getUserGivenDurationForAllLog(
                self.userGivenDurationWarnLogFileFP)

        if self._info:
            self.durationLogInfoFileName = 'USERGIVEN_' + \
                self.logTimeDuration + '_MINUTES_INFO_LOG.log'
            self.userGivenDurationInfoLogFileFP = open(
                self.seg_log_prg_name+"/" + self.caseId + "_" + self.durationLogInfoFileName, "w")

            self.getUserGivenDurationInfoLog()
            self.getUserGivenDurationForAllLog(
                self.userGivenDurationInfoLogFileFP)

        if self._trace:
            self.getUserGivenDurationTraceLog()

        if self._debug:
            self.durationLogDebugFileName = 'USERGIVEN_' + \
                self.logTimeDuration + '_MINUTES_DEBUG_LOG.log'
            self.userGivenDurationDebugLogFileFP = open(
                self.seg_log_prg_name+"/" + self.caseId + "_" + self.durationLogDebugFileName, "w")

            self.getUserGivenDurationDebugLog()
            self.getUserGivenDurationForAllLog(
                self.userGivenDurationDebugLogFileFP)
    ...
    def removeDuplicateFromFiles(self):
        ...
        if self._warn:
            ...
            if platform.system() == "Linux":
                warnFile = open(self.seg_log_prg_name+"/" +
        ...
        if self.debug:
            ...
            if platform.system() == "Linux":
                debugFile = open(self.seg_log_prg_name + "/" +
        ...
        if self.info:
            ...
            if platform.system() == "Linux":
                infoFile = open(self.seg_log_prg_name+"//" +
        ...
        if self.trace:
            ...
            if platform.system() == "Linux":
                traceFile = open(self.seg_log_prg_name +
