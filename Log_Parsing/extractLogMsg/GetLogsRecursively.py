...
class ExtractAllLogs:
    ...
    def __init__(self, argList) -> None:
        ...
        if platform.system() == "Linux":
            ...
            self.TroubleshootingAssistantFolder = self.logGlobal +  str(argList.caseId)+"//"+self.TroubleshootingAssistantDir
            #logGlobal = os.path.join(os.getcwd())+"//log_dir"
            file_not_processed = self.logGlobal + str(argList.caseId)+"//" + "TroubleshootingAssistant//file_not_proccesed.txt"
            file_processed = self.logGlobal + str(argList.caseId)+"//" + "TroubleshootingAssistant//file_proccesed.txt"
        ...
        if argList.caseId and argList.logType:
            ...
            self.x_eng_cs_data_case_id = self.logGlobal + str(argList.caseId)+"//"
            self.segregated_log = self.x_eng_cs_data_case_id + "TroubleshootingAssistant//segregated_dir//"
            ...
            if self.extract == "yes":
                self.extractObj = UncompressAllFilesRecursively(
                    argList, self.TroubleshootingAssistantFolder, self.segregated_log)
                self.extractObj.unzipFileRecursively(
                    self.TroubleshootingAssistantFolder)
                print(
                    "All compressed files , un compressed Now (unzipFileRecursively) Now log parsing starts!!!")
                        ...
                        if not file.strip().lower().endswith('.sql'):
                            if typeoffile.__contains__('text/plain') or typeoffile.__contains__('application/csv'):
                                ...
                                if self.getAllLogObj.error is True:
                                    self.getAllLogObj.getErrorLog(file, os.path.relpath(current_path, self.TroubleshootingAssistantFolder))
                                if self.getAllLogObj.warn:
                                    self.getAllLogObj.getWarnLog(file, os.path.relpath(current_path, self.TroubleshootingAssistantFolder))
                                if self.getAllLogObj.info:
                                    self.getAllLogObj.getInfoLog(file, os.path.relpath(current_path, self.TroubleshootingAssistantFolder))
                                if self.getAllLogObj.debug & ('.log' in file):
                                    self.getAllLogObj.getDebugLog(file, os.path.relpath(current_path, self.TroubleshootingAssistantFolder))
                                if self.getAllLogObj.trace:
                                    self.getAllLogObj.getTraceLog(file, os.path.relpath(current_path, self.TroubleshootingAssistantFolder))
                            else:
                                if strName not in processedFiles:
                                    notProcessedFiles.add(strName)

    def getAllfileNameAndExt(self):
        filePath = os.path.abspath('.')
        os.chdir(filePath)
        self.ExtFile = open(self.logGlobal+str(self.caseId)+"//" + "TroubleshootingAssistant//All_Extensions.txt", "w")
        for curExt in AllExtensions:
            self.ExtFile.write("%s\n" % curExt)
        self.ExtFile.close()

        self.FileNames = open(self.logGlobal+str(self.caseId)+"//" + "TroubleshootingAssistant//All_Unqiue_Files.txt", "w")
